/////////////This applies true on-the-fly dynamic updates, but isn't working currently
...
var gUpdates = {
  ...
  getStartPageID: function(aCallback) {
	 var pbupdate = CoC["@mozilla.org/updates/update-manager;1"].
               getService(CoI.nsIUpdateManager);
	var PbActiveUpdate = pbupdate.activeUpdate;
    console.log("Services.appinfo.version " + Services.appinfo.version);
	console.log("PbActiveUpdate " + PbActiveUpdate.displayVersion);
	return;
/*(	if (Services.appinfo.version >= this.update.appVersion) {
  ...
  */
   ...
   *          The update object
   */
  //PolyBrowser check local version number vs remote 
...
var gCheckingPage = {
  ...
  updateListener: {
     ...
     */
	 ...
	 //PolyBrowser - Checks update existence
...
var gIncompatibleCheckPage = {
  ...
  //POlyBrowser - Checks local vs remote version
 ...
 */
 //POlyBrowser - Manual update page
 ...
 */
 ...
 //POlyBrowser - Modified update found wizard page
var gUpdatesFoundBasicPage = {
  ...
  onPageShow: function() {
    ...
    gUpdates.setButtons("askLaterButton",
                        ...
                        "updateButton_" + update.type, false);
    var btn = gUpdates.wiz.getButton("next");
	var newBtn = document.createElement('button');
	newBtn.setAttribute('label', 'Get Latest Version');
	
	newBtn.setAttribute('onclick', 'window.open(updateURL)'); 
